...
from redwake.agents.factory import build_redwake_agent, make_child_factory
...
from redwake.core.execution import (
    respawn_subagents,
    run_agent_loop,
)
from redwake.core.execution import (
...
from redwake.core.inputs import (
    DEFAULT_MAX_TURNS,
    build_root_task,
    build_scope_context,
    make_model_settings,
...
from redwake.core.sessions import open_agent_session
from redwake.runtime import session_manager
from redwake.telemetry.logging import set_scan_id, setup_scan_logging
...
async def run_strix_scan(
    *,
    ...
    """Run or resume one RedWake scan against a sandbox."""
    if scan_id is None:
        scan_id = f"scan-{uuid.uuid4().hex[:8]}"

    # Start license heartbeat (best-effort, never blocks scan)
    if not os.environ.get("REDWAKE_LICENSE_BYPASS"):
        try:
            from redwake.license import start_heartbeat, stop_heartbeat
            start_heartbeat(scan_id)
        except Exception:
            pass  # heartbeat is best-effort; never crash scan on license issues
    ...
    logger.info(
        "%s RedWake scan %s (image=%s, max_turns=%d, interactive=%s, run_dir=%s)",
    ...
    if not resolved_model:
        raise RuntimeError(
            "No LLM model configured. Set REDWAKE_LLM env or pass model= to run_strix_scan().",
        )
    ...
    from redwake.tools.notes.tools import hydrate_notes_from_disk
    from redwake.tools.todo.tools import hydrate_todos_from_disk
    ...
    try:
        ...
        run_config = RunConfig(
            ...
            model_provider=RedWakeProvider(),
        ...
        root_agent = build_redwake_agent(
            name="redwake",
            skills=skills,
            is_root=True,
            scan_mode=scan_mode,
        ...
        logger.warning(
            ...
            "Resume with 'redwake --resume %s' once the limit clears.",
            scan_id,
            exc,
            scan_id,
        )
        ...
        logger.exception("RedWake scan %s failed", scan_id)
        if root_id is not None:
            await coordinator.cancel_descendants(root_id)
            with contextlib.suppress(Exception):
        ...
        logger.info("RedWake scan %s done", scan_id)
        teardown_logging()
