...
- **Implementation:** `agents.sandbox.capabilities.tools.view_image.ViewImageTool`
  (upstream `agents` SDK)
- **Wired in:** `redwake/agents/factory.py` — added per-run via the SDK
  `Filesystem` capability.
- **RedWake defaults:** screenshots default to
...
- **Skill:** screenshot workflow lives in `redwake/skills/tooling/agent_browser.md`.
- **Recovery:** vision-not-supported model rejections are auto-recovered
  via `redwake.core.sessions.strip_all_images_from_session`, invoked from
  `redwake/core/execution.py`.
