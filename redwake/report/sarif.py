"""SARIF 2.1.0 output for RedWake vulnerability reports.

Builds a GitHub code-scanning compatible SARIF document from RedWake findings
...
Design notes:
  * Rules are keyed on CWE (``id = CWE-NNN``), falling back to CVE, then
    ...
    RedWake output variants (``CWE-306``, ``cwe: 306``, ``306``) to the
    canonical ``CWE-NNN`` form so dedup works across runs.
  * SARIF only has three levels (error / warning / note). RedWake's five
    severities collapse into them. The raw severity label and CVSS score
    survive in ``result.properties.redwake`` for downstream tools that can
...
from redwake.report.compliance import enrich_finding
...
SARIF_VERSION = "2.1.0"
TOOL_NAME = "RedWake"
TOOL_INFORMATION_URI = "https://redwake.rf.gd"
...
# SARIF only has three result levels; RedWake's five severities collapse here.
# Original label survives in ``result.properties.redwake.severity``.
_SEVERITY_TO_LEVEL = {
    "critical": "error",
    "high": "error",
    "medium": "warning",
...
def _apply_repository_context(run: dict[str, Any], context: dict[str, Any]) -> None:
    ...
    ``automationDetails.id`` categorises the run (``redwake/<owner>/<repo>``)
    so multiple RedWake runs against the same repo reconcile rather than pile
    ...
    if full_name:
        run["automationDetails"] = {"id": f"redwake/{full_name}"}
...
def _build_rule(rule_id: str, report: dict[str, Any]) -> dict[str, Any]:
    """Build a SARIF rule descriptor from a RedWake finding."""
    ...
    # Compliance metadata (OWASP / CWE / PCI-DSS / NIST 800-53) for
    # downstream consumers (audit dashboards, compliance tooling).
    compliance_block = _compliance_properties(report)
    if any(compliance_block.values()):
        properties["compliance"] = compliance_block
...
def _result_properties(
    ...
    *,
    ...
    """RedWake-specific metadata for downstream consumers.

    The top-level ``security-severity`` matches GitHub code-scanning's
    expected property. RedWake-specific fields are namespaced under
    ``redwake`` so generic SARIF consumers don't see them by default.
    """
    properties: dict[str, Any] = {
        "security-severity": _security_severity(report),
    }
    if class_fingerprint:
        ...
        # filter alerts by it without parsing the nested redwake.* tree.
        properties["redwake_vuln_class_hash"] = class_fingerprint
    if is_synthetic:
        # Top-level so reviewers + downstream automation can filter
        # synthetic-anchored alerts without parsing the nested redwake.*
        # tree.
        properties["synthetic_location"] = True

    redwake: dict[str, Any] = {}
    for key in (
        "id",
        "severity",
        "cvss",
...
def _build_fixes(report: dict[str, Any]) -> list[dict[str, Any]] | None:
    ...
    RedWake findings carry the suggested change inline on each code
...
def _rule_id(report: dict[str, Any]) -> str:
    ...
    CWE values are normalised from RedWake output variants (``CWE-306``,
    ...
    title = _string_value(report.get("title")) or "redwake-finding"
    ...
    *,
    ...
    Computed from primitives that don't depend on LLM prose stability:
      ...
      - first SARIF location's URI + startLine — these come from
        RedWake's ``code_locations[].file`` and ``start_line`` which
    ...
    if uri:
        ...
        if start_line is not None:
            ...
            # line because RedWake code_locations carry the SINK line,
...
def _sarif_level(severity: Any) -> str:
    """Map RedWake severity labels to SARIF result levels."""
    normalised = (_string_value(severity) or "").lower()
    return _SEVERITY_TO_LEVEL.get(normalised, "note")


def _compliance_properties(report: dict[str, Any]) -> dict[str, Any]:
    """Build the compliance block for SARIF ``properties.compliance``.

    Delegates to ``enrich_finding`` so the static mapping tables in
    ``redwake.report.compliance`` remain the single source of truth.
    """
    return enrich_finding(report).get("compliance", {})
...
def _slugify(value: str) -> str:
    ...
    return slug or "redwake-finding"
