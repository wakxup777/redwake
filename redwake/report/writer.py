"""Artifact writers for RedWake scan reports."""
...
from redwake.core.paths import run_record_path
...
def write_pdf_report(
    ...
    """Generate a PDF report alongside the Markdown one.

    Returns True on success, False if reportlab is not installed
    (graceful: logs a warning, no crash).

    Reads compliance annotations from the findings and emits a structured
    audit-ready PDF with cover page, summary table, and per-finding detail.
    """
    # reportlab is an optional dependency — import lazily so the rest of
    # the report pipeline still works when reportlab isn't installed.
    try:
        from redwake.report.compliance import enrich_finding  # noqa: PLC0415
        from redwake.report.pdf import write_pdf_report as _render  # noqa: PLC0415
    except ImportError as exc:
        logger.warning("PDF generation skipped (missing dependency): %s", exc)
        return False

    enriched = [enrich_finding(f) for f in findings]
    scan_date = datetime.now(UTC).strftime("%Y-%m-%d %H:%M UTC")
    out_path = run_dir / "penetration_test_report.pdf"
    try:
        _render(enriched, target=target, scan_date=scan_date, output_path=out_path)
