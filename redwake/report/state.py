...
from redwake.core.paths import run_dir_for
from redwake.report.sarif import write_sarif
from redwake.report.usage import LLMUsageLedger
from redwake.report.writer import (
    read_run_record,
...
class ReportState:
    ...
    persistence. This store keeps only RedWake-owned scan artifacts and
    ...
    def _save_artifacts(self) -> None:
        ...
        try:
            ...
            if self.final_scan_result:
                ...
                # Generate PDF report alongside Markdown if requested via env.
                # (CLI flag --report-format=pdf is wired in main.py and propagates
                # REDWAKE_REPORT_FORMAT, see interface/main.py.)
                report_format = os.environ.get("REDWAKE_REPORT_FORMAT", "md,sarif")
                if "pdf" in {fmt.strip().lower() for fmt in report_format.split(",")}:
                    write_pdf_report(
                        run_dir,
                        self.vulnerability_reports or [],
                        target=str(getattr(self, "scan_config", {}).get("target", "(unknown)")),
                    )
