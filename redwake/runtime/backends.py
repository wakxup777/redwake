"""Sandbox backend registry — selected via REDWAKE_RUNTIME_BACKEND (default: docker)."""
...
async def _docker_backend(
    *,
    ...
    Uses :class:`RedWakeDockerSandboxClient` to inject NET_ADMIN /
    ...
    would call it too, but RedWake manages session lifetime explicitly via
    ...
    from redwake.runtime.docker_client import RedWakeDockerSandboxClient

    client = RedWakeDockerSandboxClient(docker.from_env())
    client.redwake_bind_mounts = bind_mounts or []
...
def get_backend(name: str) -> SandboxBackend:
    ...
    if backend is None:
        ...
        raise ValueError(
            f"Unknown REDWAKE_RUNTIME_BACKEND: {name!r} (supported: {supported})",
        )
