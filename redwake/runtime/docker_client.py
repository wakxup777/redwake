"""RedWakeDockerSandboxClient — preserves the image's ENTRYPOINT and adds
...
class RedWakeDockerSandboxClient(DockerSandboxClient):
    ...
    redwake_bind_mounts: list[dict[str, Any]] = []  # overridden per-instance in backends.py
    ...
    async def _create_container(
        ...
        *,
        ...
        # RedWake delta from the SDK body: drop ``entrypoint`` override and
        ...
        # RedWake injections — append, don't overwrite, so FUSE/SYS_ADMIN survives.
        ...
        # RedWake injection: host bind mounts (e.g. large repos passed via --mount)
        # that bypass the SDK's file-by-file LocalDir copy.
        bind_mounts = getattr(self, "redwake_bind_mounts", ())
        if bind_mounts:
