...
Examples:
  ...
  redwake --target https://app.com --instruction-file /path/to/detailed_instructions.md
    ...
    parser.add_argument(
        "--config",
        type=str,
        help="Path to a custom config file (JSON) to use instead of ~/.redwake/cli-config.json",
    )
...
def main() -> None:
    ...
    # 1. Anti-debug (silent exit if debugger/strace/gdb detected)
    from redwake.license.anti_debug import check as anti_debug_check
    anti_debug_check()

    # 2. License verification (fail-fast: no key = no run)
    if not os.environ.get("REDWAKE_LICENSE_BYPASS"):
        try:
            from redwake.license import verify_or_exit
            verify_or_exit()
        except Exception as e:
            console = Console()
            console.print(f"[bold red]License verification failed:[/] {e}")
            console.print("[dim]Contact your administrator to obtain a license key.[/]")
            console.print("[dim]Set REDWAKE_LICENSE_KEY='REDWAKE-LIC-...' and re-run.[/]")
            sys.exit(1)
