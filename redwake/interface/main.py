...
from redwake.config import (
    apply_config_override,
    load_settings,
    persist_current,
)
from redwake.config.models import (
    RedWakeProvider,
    configure_sdk_model_defaults,
    is_known_openai_bare_model,
)
from redwake.core.paths import run_dir_for, runtime_state_dir
from redwake.interface._notify import _notify_scan_complete
from redwake.interface._save_report import save_report_artifacts as _save_report_artifacts
from redwake.interface.cli import run_cli
from redwake.interface.tui import run_tui
...
from redwake.report.state import get_global_report_state
from redwake.report.writer import read_run_record, write_run_record
from redwake.telemetry import posthog, scarf
from redwake.telemetry.logging import configure_dependency_logging
...
def validate_environment() -> None:
    ...
    if not settings.llm.model:
        missing_required_vars.append("REDWAKE_LLM")
    ...
    if missing_required_vars:
        ...
        for var in missing_required_vars:
            if var == "REDWAKE_LLM":
                ...
                error_text.append("REDWAKE_LLM", style="bold cyan")
                error_text.append(
        ...
        if missing_optional_vars:
            ...
            for var in missing_optional_vars:
                ...
                elif var == "REDWAKE_REASONING_EFFORT":
                    error_text.append("• ", style="white")
                    error_text.append("REDWAKE_REASONING_EFFORT", style="bold cyan")
                    error_text.append(
        ...
        error_text.append("\nExample setup:\n", style="white")
        error_text.append("export REDWAKE_LLM='openai/gpt-5.4'\n", style="dim white")
...
async def warm_up_llm() -> None:
    ...
    try:
        ...
        model = RedWakeProvider().get_model(raw_model)
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="RedWake Multi-Agent Cybersecurity Penetration Testing Tool",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Web application penetration test
  redwake --target https://example.com

  # GitHub repository analysis
  redwake --target https://github.com/user/repo
  redwake --target git@github.com:user/repo.git

  # Local code analysis
  redwake --target ./my-project

  # Large local repository (bind-mounted read-only instead of copied)
  redwake --mount ./huge-monorepo

  # Domain penetration test
  redwake --target example.com

  # IP address penetration test
  redwake --target 192.168.1.42

  # Multiple targets (e.g., white-box testing with source and deployed app)
  redwake --target https://github.com/user/repo --target https://example.com
  redwake --target ./my-project --target https://staging.example.com --target https://prod.example.com

  # Custom instructions (inline)
  redwake --target example.com --instruction "Focus on authentication vulnerabilities"

  # Custom instructions (from file)
  redwake --target example.com --instruction-file ./instructions.txt
  redwake --target https://app.com --instruction-file /path/to/detailed_instructions.md
        """,
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "~/.config/redwake/cli-config.json"
        ),
    )
    ...
    parser.add_argument(
        ...
        metavar="RUN_NAME",
        help=(
            "Resume a prior scan by its run name (the dir under ./redwake_runs/). "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Start RedWake MCP server (JSON-RPC 2.0 at /mcp) and exit. "
            "Use --mcp-port, --mcp-host, --mcp-auth-token to configure. "
            "Clients (Claude Code, Cursor, Windsurf) can call RedWake tools."
        ),
    )
    parser.add_argument(
        "--mcp-host",
    ...
    parser.add_argument(
        ...
        type=int,
        default=9877,
        help="MCP server bind port (default: 9877)",
    )
    parser.add_argument(
        "--mcp-auth-token",
        type=str,
        default="",
        help=(
            "Bearer token for MCP server auth (or REDWAKE_MCP_TOKEN env var). "
            "Empty = no auth (localhost dev only)."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Copy final report (markdown + JSON) to PATH after scan completes. "
            "Useful when running interactively in TUI (mouse copy may not work in terminal). "
            "Example: --save-report /tmp/scan.md"
        ),
    )
    ...
    parser.add_argument(
        ...
        default="md,sarif",
        help=(
            "Comma-separated list of formats to emit. Valid: md, sarif, pdf. "
            "Default: md,sarif. Example: --report-format md,sarif,pdf"
        ),
    )
    ...
    if args.resume:
        ...
        if oversized:
            ...
            parser.error(
                ...
                "(set REDWAKE_MAX_LOCAL_COPY_MB to change it). Re-run with "
...
def display_completion_message(args: argparse.Namespace, results_path: Path) -> None:
    ...
    if not scan_completed:
        ...
        resume_text.append("  ")
        resume_text.append(f"redwake --resume {args.run_name}", style="#22c55e")
        panel_parts.extend(["\n", resume_text])
    ...
    console.print()
    console.print(
        "[#60a5fa]redwake.rf.gd[/]  [dim]·[/]  "
        "[#60a5fa]docs.redwake.rf.gd[/]  [dim]·[/]  "
        "[#60a5fa]redwake.rf.gd/contact[/]"
    )
    console.print()
...
def main() -> None:
    ...
    # Expose --report-format to ReportState via env var. (ReportState is
    # constructed in many places; reading a single env var keeps coupling
    # minimal.)
    if getattr(args, "report_format", None):
        os.environ["REDWAKE_REPORT_FORMAT"] = args.report_format
    ...
    # All of the following runs AFTER the asyncio event loop has fully closed.
    # Earlier placements (inside the `finally:` above) tried to spawn webhook
    # threads while the loop was still being torn down, which produced
    # `RuntimeError: scan loop is not ready` from the SDK's _notify path.
    results_path = run_dir_for(args.run_name)
    display_completion_message(args, results_path)

    if args.save_report:
        _save_report_artifacts(args.save_report, results_path)

    # Fire webhook notification (if configured). Runs in background thread
    # so scan completion is not blocked on webhook latency.
    report_state_final = get_global_report_state()
    if report_state_final:
        _notify_scan_complete(exit_reason, report_state_final)
