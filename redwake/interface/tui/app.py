...
from redwake.core.hooks import BudgetExceededError
from redwake.core.runner import run_strix_scan
from redwake.interface.tui.live_view import TuiLiveView
from redwake.interface.tui.messages import send_user_message_to_agent
from redwake.interface.tui.renderers import render_tool_widget
from redwake.interface.tui.renderers.agent_message_renderer import AgentMessageRenderer
from redwake.interface.tui.renderers.user_message_renderer import UserMessageRenderer
from redwake.interface.utils import build_tui_stats_text
...
def get_package_version() -> str:
    try:
        return pkg_version("redwake-agent")
...
class ChatTextArea(TextArea):  # type: ignore[misc]
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._app_reference: RedWakeTUIApp | None = None

    def set_app_reference(self, app: "RedWakeTUIApp") -> None:
...
class SplashScreen(Static):  # type: ignore[misc]
    ...
    def _build_start_line_text(self, phase: int) -> Text:
        full_text = "Starting RedWake Agent"
...
class HelpScreen(ModalScreen):  # type: ignore[misc]
    def compose(self) -> ComposeResult:
        yield Grid(
            Label("RedWake Help", id="help_title"),
            Label(
                "F1        Help\nCtrl+Q/C  Quit\nESC       Stop Agent\n"
...
class QuitScreen(ModalScreen):  # type: ignore[misc]
    def compose(self) -> ComposeResult:
        yield Grid(
            Label("Quit RedWake?", id="quit_title"),
            Grid(
...
class RedWakeTUIApp(App):  # type: ignore[misc]
    ...
    def _merge_renderables(renderables: list[Any]) -> Text:
        ...
        for i, item in enumerate(renderables):
            if i > 0:
                combined.append("\n")
            RedWakeTUIApp._append_renderable(combined, item)
        return RedWakeTUIApp._sanitize_text(combined)
    ...
    def _append_renderable(combined: Text, item: Any) -> None:
        ...
        if isinstance(item, Text):
            combined.append_text(RedWakeTUIApp._sanitize_text(item))
        elif isinstance(item, Group):
            for j, sub in enumerate(item.renderables):
                if j > 0:
                    combined.append("\n")
                RedWakeTUIApp._append_renderable(combined, sub)
        else:
            inner = getattr(item, "content", None) or getattr(item, "renderable", None)
            if inner is not None:
                RedWakeTUIApp._append_renderable(combined, inner)
            else:
            ...
            try:
                ...
                try:
                    if not self._scan_stop_event.is_set():
                        image = load_settings().runtime.image or "redwake-sandbox:latest"
    ...
    def _clean_copied_text(text: str) -> str:
        ...
        for line in lines:
            ...
            if stripped in RedWakeTUIApp._DECORATIVE_LINES:
                continue
            if stripped and all(c == "─" for c in stripped):
                continue
            out = line
            for prefix in RedWakeTUIApp._ICON_PREFIXES:
