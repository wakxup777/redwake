---
title: "CLI Reference"
description: "Command-line options for RedWake"
---

## Basic Usage

```bash
redwake --target <target> [options]
```
...
<ParamField path="--mount" type="string">
  ...
  RedWake copies local `--target` directories into the sandbox one file at a time, which stalls on very large trees. When a local target exceeds the copy limit (see `REDWAKE_MAX_LOCAL_COPY_MB`, default 1024 MB) RedWake exits early and asks you to re-run with `--mount`.
...
<ParamField path="--config" type="string">
  Path to a custom config file (JSON) to use instead of `~/.redwake/cli-config.json`.
</ParamField>
...
<ParamField path="--max-budget-usd" type="number">
  ...
  **Limitations**
  ...
  - For LiteLLM-routed models, RedWake enables streaming success callbacks to
...
redwake --target https://app.com --instruction "Use credentials: user:pass"

# Focused testing
redwake --target api.example.com --instruction "Focus on IDOR and auth bypass"

# CI/CD mode
redwake -n --target ./ --scan-mode quick

# Force diff-scope against a specific base ref
redwake -n --target ./ --scan-mode quick --scope-mode diff --diff-base origin/main

# Multi-target white-box testing
redwake -t https://github.com/org/app -t https://staging.example.com
