...
RedWake offers three scan modes to balance speed and thoroughness.

## Quick

```bash
redwake --target ./app --scan-mode quick
```
...
```bash
redwake --target ./app --scan-mode standard
```
...
```bash
redwake --target ./app --scan-mode deep
```
